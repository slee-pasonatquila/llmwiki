# Sync Command (`/llmwiki_sync`)

`wiki/` 配下のすべての Concept ドキュメントおよび `.changelogs/` の変更断片を読み込み、`wiki/index.md`（全体索引）、`wiki/log.md`（統合変更履歴）、`wiki/graph.json` および `wiki/graph.mermaid`（ナレッジグラフ）を一括自動再構築します。
