...
description: "一次資料 (Office, PDF, SQL, text 等) を自動クレンジング（不要空行・空セル削除）して取り込み、Google OKF (Open Knowledge Format v0.1) 準拠の Concept ドキュメントとして wiki/ 配下に分類・出力し、index.md および log.md を自動更新するスキル。"
---

# LLM Wiki Ingest Skill

このスキルは、ユーザーから「資料をWikiに追加して」と指示された際に、`raw/` 配下に配置された各種書類（顧客要望、要件定義書、概要設計書、詳細設計書、議事録など）を読み込み、**自動クレンジング（空セル・不要空行の削除等）** を施した上で、**Google OKF (v0.1)** 仕様に従って構造化された知識ファイル（Concept ドキュメント）として `wiki/` に編纂・配置します。
...
| **その他** | `wiki/99_others/` | `Glossary`, `Meeting Minutes`, `Runbook` | プロジェクト用語集、議事録、運用マニュアル |

---

## Ingest の作業手順（自動実行パイプライン）

### 1. ドキュメントの変換と自動クレンジング
...
timestamp: 2026-08-14T15:30:00Z  # 任意: ISO 8601 形式
resource: raw/04_detailed_designs/user_schema.xlsx # 任意: 一次資料
status: Approved                 # 任意: Draft / In Review / Approved
---
...
# 関連概念
* 認証方式: [Authentication Architecture](../03_basic_designs/authentication.md)
* 注文テーブル: [Orders Table](../04_detailed_designs/table_orders.md)

# Citations
[1] [User Schema Excel](raw/04_detailed_designs/user_schema.xlsx#Sheet1)
```
...
- `wiki/log.md` の先頭に本日の日付で取り込み履歴を追記。
